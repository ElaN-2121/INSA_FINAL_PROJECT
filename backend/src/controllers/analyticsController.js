...
async function getSystemOverview(req, res) {
  const overview = await analyticsRepository.getSystemOverview();
  return success(res, overview, 'System overview retrieved successfully');
}

async function getLeaderboard(req, res) {
  const leaderboard = await analyticsRepository.getInstitutionLeaderboard();
...
async function getDepartmentStats(req, res) {
  ...
  const stats = await analyticsRepository.getDepartmentBreakdown(institutionId || null);
