...
import { Home, Search, MessageCircle, Clock, FileText, LogOut, UserCircle } from 'lucide-react';
import { useAuth } from '../context/AuthContext.jsx';
import logo from '../assets/ethiocred-logo.png';

const navLinks = [
  { to: '/dashboard', label: 'Dashboard', icon: <Home size={18} /> },
  { to: '/verify', label: 'Verify Credential', icon: <Search size={18} /> },
  { to: '/request', label: 'Request Verification', icon: <MessageCircle size={18} /> },
  { to: '/history', label: 'History', icon: <Clock size={18} /> },
  { to: '/request-status', label: 'Request Status', icon: <FileText size={18} /> },
];
